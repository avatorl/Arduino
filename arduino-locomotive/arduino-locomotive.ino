  ...
  // Physical direction is independent of the selected manual direction. A Stop/CH reset must
  // not erase it, or an opposite jog could reverse the still-spinning motor without coasting.
  ...
  void initI2cBus() {
    ...
    Wire.begin();
    // Match the verified standalone VL53L0X setup. The shared train wiring and pull-ups are more
    // reliable at standard-mode I2C than at 400 kHz fast mode.
    ...
    if (batteryVoltage <= BATTERY_LOW_SHUTDOWN_MV) {
      ...
      for (uint8_t attempt = 1; attempt < (BATTERY_LOW_CONFIRMATION_COUNT + 2U); ++attempt) {
        ...
        if (retry <= BATTERY_LOW_SHUTDOWN_MV) {
          ...
          confirmations = 0;
          break;  // one healthy reading exonerates the pack; no need to keep polling
        }
      }
      if (lastPlausible != 0U) batteryVoltage = lastPlausible;
      if (confirmations >= BATTERY_LOW_CONFIRMATION_COUNT) {
        enterBatteryShutdown(true);
        return;
      }
    }
    if (batteryVoltage <= BATTERY_LOW_WARNING_MV && batteryVoltage >= BATTERY_IMPLAUSIBLE_MV) {
      enterBatteryWarning();
    }
    #endif
  ...
  void loop() {
    ...
    // When distance debugging is enabled, the color-sensor illumination toggle on A2 explicitly
    // requests live ToF samples even while auto-distance control is off.
